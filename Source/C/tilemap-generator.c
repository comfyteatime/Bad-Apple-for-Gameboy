// This version generates tilemaps while accounting for the full 32x32 map (filling offscreen tiles with id 0)
// These tilemaps can be used to display a single image on the screen
...
int main(int argc, char *argv[]){
    ...
    for(int row=0; row<TILEHEIGHT; row++) {
        for(int column=0; column<TILEWIDTH; column++) {
            best_score = 99999; // To ensure score updates first time
        ...
        for(int h=0; h<FULLTILEMAPLENGTH-TILEWIDTH; h++) fwrite(&empty_tile, sizeof(uint8_t), 1, FTilemap); // write 0 to offscreen tiles
    }
